	...
	public boolean onCommand(CommandSender sender, Command arg1, String label, String[] args) {
		...
		if (args.length >= 1) {
			switch (args[0].toLowerCase()) {
				case "help": {
					sender.sendMessage(ChatColor.GREEN + "wgex reload - Перезагрузить конфигурацию");
				...
				case "reload": {
					config.loadConfig();
					sender.sendMessage(ChatColor.GREEN + "Конфигурация перезагружен!");
					return true;
				}
				case "search": {
				...
				case "removemember": {
					...
					sender.sendMessage(ChatColor.GREEN + "Игрок удалён из списков участников всех регионов");
					return true;
				}
			}
		}
